...
import com.example.bagmarket.ui.features.product.MainTextField
...
import com.example.bagmarket.util.styleTime
        ...
        Column(
            ...
            Button(
                onClick = {
                    Toast.makeText(
                        context,
                        "dear $name , Hope to see you again ",
                        Toast.LENGTH_SHORT
                    ).show()
                    viewModel.signOut()
...
fun ShowDataSection(subject: String, textToShow: String, OnLocationClicked: (() -> Unit)?) {
    Column(
        modifier = Modifier
            ...
            .clickable { OnLocationClicked?.invoke() }, horizontalAlignment = Alignment.Start
    ) {
...
@Composable
fun ProfileToolbar(name:String,OnBackClicked: () -> Unit) {
